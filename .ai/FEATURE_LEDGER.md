...
- `apps/mobile/src/lib/push-server.ts` centralizes server-side Web Push delivery and applies `NotificationPreference` category settings plus timezone-aware quiet hours.
- Message creation and check-in creation/review now trigger Push asynchronously while retaining in-app notifications.
- Message sends and check-in writes are rate limited through the existing Upstash/fallback limiter.
- Source-level presence of these integrations is `E2_STATIC`; real browser/device delivery remains unverified until an actual deployment and supported client are exercised.
